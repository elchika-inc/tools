...
- **粒度をファイル単位にしたことによる被覆の後退（残余）**: `vp check` はエラーの**報告対象を渡したファイルに限定する**。実測（2026-08-17）: `vp check --no-fmt --no-lint apps/json-formatter/src/App.tsx` は exit 0（`Found no type errors in 1 file`）だが、同じアプリの別ファイルを1つ足すと exit 1（`Found 1 error and 0 warnings in 2 files`）。したがって **`apps/foo/src/utils/bar.ts` のシグネチャを変える PR が、同じアプリの未変更の `App.tsx` を壊しても CI は green になる**（変更ファイル自身は型的に正しいため）。ディレクトリ単位だった時点ではこの型の破壊は赤で顕在化していたので、**ラウンド1の設計と比べれば被覆は縮んでいる**（ただしディレクトリ単位は 21/21 が赤で運用できないため、戻すことは別の後退になる）。`Build changed apps` もこれを拾わない —— `vp build` は型エラーがあっても exit 0 を返し、wasm 依存3アプリはそもそも skip される（RISK-006）。**この経路の検出はデプロイ時の `bash scripts/build-all.sh` まで遅延する。**
...
- anchor: `ci.yml` の `Build changed apps` ステップが、**全アプリのビルドが成功したときに**「ビルドしたアプリ: N 件 / skip: M 件 / 対象外: K 件」を出力する（検出したすべてが3つのどれかに計上され、サイレントな打ち切りにならない）。ビルドが1件でも失敗した場合は `set -e` でこの集計に到達せずステップが赤くなるため、いずれの場合も「無言で通る」ことはない。ビルドが壊れたアプリ・パッケージが `main` へ入った場合は `scripts/build-all.sh` が `set -e` で停止し、デプロイ作業の時点で顕在化する。
...
- anchor: `Build changed apps` ステップが skip のたびに `::warning::` を出す（この warning はビルドの成否によらず skip の時点で出る）。これら3アプリのビルド破壊は `scripts/build-all.sh`（Step 0 で `wasm-utils` を先にビルドする）が `set -e` で停止することにより、デプロイ作業の時点で顕在化する。
...
- 解除条件: 司令塔が ①ruleset を新設し required status check に `ci` を指定 ②strict 設定または merge queue を有効化 —— を完了した時点で `mitigated` にし、あわせて `branch_policy` を `unprotected` から `protected` へ改める。**`allow_auto_merge` は解除の必須条件ではない**（§5「マージ機構」は `--auto` を使わず `gh pr checks` で全 check の green を実測してマージする経路も認めているため。`--auto` を使う場合にのみ必要）。**逆に、これらの設定を実施しない判断になった場合は `merge_policy` を `human` へ戻す**（宣言だけを残さない）。
