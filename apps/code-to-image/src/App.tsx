...
export default function App() {
  ...
  const handleGenerate = () => {
    if (!code.trim()) {
      toast.add({ title: 'Please enter some code', type: "error" });
      return;
    }
    try {
      ...
      toast.add({ title: 'Image generated successfully' });
    } catch {
      toast.add({ title: 'Failed to generate image', type: "error" });
    }
  };
  ...
  const handleCopyImage = async () => {
    ...
    try {
      ...
      toast.add({ title: 'Image copied to clipboard' });
    } catch {
      toast.add({ title: 'Failed to copy image', type: "error" });
    }
  };
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <textarea
                      id="code"
                      className="flex min-h-[250px] w-full rounded-md border border-input bg-background px-3 py-2 text-sm font-mono placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-[3px] focus-visible:ring-ring disabled:cursor-not-allowed disabled:opacity-50 resize-none"
