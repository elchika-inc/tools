...
import { morseToText, textToMorse } from "@/utils/morse";
...
export default function App() {
  ...
  const handleEncode = () => {
    try {
      ...
      toast.add({ title: "Encoded to Morse" });
    } catch {
  ...
  const handleDecode = () => {
    try {
      ...
      toast.add({ title: "Decoded from Morse" });
    } catch {
      toast.add({ title: "Decoding failed", type: "error" });
    }
  };
  ...
  <div className="min-h-screen bg-background p-8">
        <main className="max-w-6xl mx-auto space-y-6">
          <header className="space-y-2">
            ...
            <h1 className="text-3xl font-bold tracking-tight">Morse Code Converter</h1>
            <p className="text-muted-foreground">テキストとモールス符号の相互変換を行います。</p>
          </header>

          <Card>
            <CardHeader>
              <CardTitle>Converter</CardTitle>
              <CardDescription>テキストまたはモールス符号を入力してください。</CardDescription>
            </CardHeader>
              ...
              <div className="grid gap-6 md:grid-cols-[1fr,auto,1fr] items-start">
                ...
                <div className="flex flex-col gap-4 justify-center pt-10">
                  <Button type="button" onClick={handleEncode} disabled={!input}>
                    Text to Morse <ArrowRight className="ml-2 h-4 w-4" />
                  </Button>
                  <Button type="button" onClick={handleDecode} variant="secondary" disabled={!input}>
                    Morse to Text <ArrowRight className="ml-2 h-4 w-4" />
                  </Button>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    id="output"
                    readOnly
                    className="flex min-h-[250px] w-full rounded-md border border-input bg-muted px-3 py-2 text-sm font-mono placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-[3px] focus-visible:ring-ring resize-none"
                    placeholder="Result..."
                    value={output}
                  />
                </div>
